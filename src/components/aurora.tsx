...
void main() {
  ...
  colors[1] = ColorStop(uColorStops[1], 0.3333333);
  colors[2] = ColorStop(uColorStops[2], 0.6666666);
...
function resolveColorStops(stops: string[] | undefined): string[] {
  const base = stops?.length ? [...stops] : [...DEFAULT_COLOR_STOPS]
  while (base.length < 4) base.push(base[base.length - 1]!)
  return base.slice(0, 4)
}

function rgbStopsFromHex(stops: string[]): number[][] {
...
export function Aurora({
  ...
  useEffect(() => {
    ...
    const colorStopsArray = rgbStopsFromHex(resolveColorStops(initial.colorStops))
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uColorStops.value = rgbStopsFromHex(
        resolveColorStops(p.colorStops),
      )
